...
interface PlayerData { wallet: string; score: number; currentQ: number; totalTime: number; shuffledQs: any[]; pendingSubmit: boolean; }  // Add pending to queue
...
io.on('connection', (socket: Socket) => {
    ...
    if (pools[poolId]) {
      if (!pools[poolId].playerList.includes(wallet)) {
        ...
        if (games[poolId]) {
          ...
          if (!playerData) {
            playerData = { wallet, score: 0, currentQ: 0, totalTime: 0, shuffledQs: [], pendingSubmit: false };
        ...
        if (pools[poolId].players >= 1 && !games[poolId]) {
          try {
            ...
            games[poolId] = { 
              ...
              players: pools[poolId].playerList.map(w => ({ wallet: w, score: 0, currentQ: 0, totalTime: 0, shuffledQs: baseShuffled.map((q, i) => ({ ...q, index: i })), pendingSubmit: false })),
              startTime: Date.now(),
              gameId: Date.now()
            };
  ...
  socket.on('submitAnswer', (data: { poolId: string; wallet: string; answer: string; qIndex: number; submitTime: number }) => {
    ...
    if (games[poolId]) {
      ...
      if (player && player.currentQ === qIndex && !player.pendingSubmit) {
        player.pendingSubmit = true;  // Queue to prevent race
        ...
        console.log(`Submit ignored for ${wallet.slice(0,6)}... in ${poolId}: qIndex ${qIndex} != currentQ ${player.currentQ} or pending.`);
      }
    }
  });
  ...
  socket.on('timeout', (data: { poolId: string; wallet: string; qIndex: number; timeoutTime: number }) => {
    ...
    if (games[poolId]) {
      ...
      if (player && player.currentQ === qIndex && !player.pendingSubmit) {
        ...
        player.currentQ += 1;
        player.pendingSubmit = false;  // Reset
        ...
        console.log(`Timeout ignored for ${wallet.slice(0,6)}... in ${poolId}: qIndex ${qIndex} != currentQ ${player.currentQ} or pending.`);
      }
    }
  });
